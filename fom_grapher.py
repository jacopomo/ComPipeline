...
r1 = [0,0.005,0.103,0.258,0.404,0.534,0.7,0.791,0.772,0.579,0.429,0.137,0.084]
r2 = [0,0.016,0.037,0.023,0.018,0.017,0.041,0.274,0.42,0.438,0.379,0.186,0.116]
###
...
for (label, y), color, marker in zip(seriesn.items(), colors, markers):
...
values = np.array(list(seriesn.values()))  # shape (n_series, n_points)
...
seriesr = {
    r'Revan reconstruction without ET': r1,
    r'Revan reconstruction with ET': r2,
}
colors = ['C0', 'C1']
markers = ['o', 's']
...
for (label, y), color, marker in zip(seriesr.items(), colors, markers):
    ax.plot(E, y, linestyle=':', marker=marker, color=color, label=label)
 
# --- Shade the regions where each model is the best (highest) ---
# Winner at each E point, then shade each interval by whichever model
# is on top there (using the midpoint of the interval as the check).
values = np.array(list(seriesr.values()))  # shape (n_series, n_points)
E_arr = np.array(E, dtype=float)
...
ax.legend()
plt.tight_layout()
plt.savefig('fom_vs_Rev.png', dpi=150)
plt.show()
